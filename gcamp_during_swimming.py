...
for x_range in x_ranges:
    with plt.rc_context({'lines.linewidth': 2, 'figure.figsize': (12,8), 'font.size':20}):
        fig, ax1 = plt.subplots()
        ax1.plot(time_duringimaging[x_range], tail_power[x_range], label='tail power')
        ax1.plot(time_duringimaging[x_range], bout_st_end[x_range] * np.mean(tail_power)*7, 'r--', label='bout calls')
        ax1.set_ylabel('tail power (au)')
        ax2 = ax1.twinx()
        ax2.plot(time_duringimaging[x_range], bend_amps_filt_imaging[x_range], 'C1', label='bend amp.')
        ax2.set_ylabel('tail bend amplitude (rad)')
        #plt.ylim((-0.2,0.2))
        plt.show()
...
for corr in range(5):
    ...
    plt.imshow(np.mean(stack_reslice, axis=0))
    plt.title(corr_order[corr])
    plt.show()
